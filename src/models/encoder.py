...
Blueprint §7:
  ...
  - Input: single-channel grayscale replicated to 3 channels (B, 3, 512, 512)
...
class PatchEmbedding(nn.Module):
    """Convolutional patch tokeniser: (B, 3, H, W) → (B, N, D).
...
class MAEEncoder(nn.Module):
    ...
    def __init__(self, pretrained_imagenet: bool = True):
        ...
        self.patch_embed = PatchEmbedding(in_channels=3, patch_size=PATCH_SIZE, embed_dim=64)
    ...
    def forward(
        ...
        Parameters
        ----------
        x : (B, 3, 512, 512)  — normalised grayscale replicated to 3 channels
        ...
        # Project 64 → 3 channels for MobileNetV3
        if not hasattr(self, "_proj_to_3ch"):
            self._proj_to_3ch = nn.Conv2d(64, 3, 1, bias=False).to(x.device)
        feat_rgb = self._proj_to_3ch(feat_upsampled)

        # Step 4: Pass through MobileNetV3 backbone (hooks capture stride-4 and stride-32)
        _ = self.backbone.features(feat_rgb)
